...
async def on_message(client, message: Message):
    if message.from_user.id in SCANNED_ID:
        ...
        try:
            ...
            await message.reply_text(NOTICE_MSG.format(BLUE_DATABASE[str(message.from_user.id)][0], message.from_user.mention(message.from_user.first_name) , BLUE_DATABASE[str(message.from_user.id)][2], BLUE_DATABASE[str(message.from_user.id)][1]))
        except Exception as e:
